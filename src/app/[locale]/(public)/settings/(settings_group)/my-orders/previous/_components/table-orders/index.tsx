			...
			<table className="min-w-full border border-text-borders">
				<thead>
					<tr className="bg-[#F2F2F2] rtl:text-right ltr:text-left ">
						<th className="p-4 text-[14px] font-normal text-text-main">{t("order_number")}</th>
						<th className="p-4 text-[14px] font-normal text-text-main">{t("Quantity")}</th>
						<th className="p-4 text-[14px] font-normal text-text-main">{t("price")}</th>
						<th className="p-4 text-[14px] font-normal text-text-main">{t("status")}</th>
				...
				<tbody>
					{orders.map((order) => (
						<tr key={order.id} className="hover:bg-gray-50">
							<td className="p-4 border-b border-text-borders">{order.order_number}</td>
							<td className="p-4 border-b border-text-borders">{order.quantity}</td>
    ...
    ${
			order.status === "complete"
				...
				: "bg-[#FFE1E1] text-red-700" // أحمر للملغي
							...
							<td className="p-4 border-b border-text-borders text-[1rem] font-bold text-normal cursor-pointer text-end ">
								<Link href={`/settings/order-details-previous/${order.id}`}> {t("view_order_details")} </Link>
							</td>
			...
			<div className="py-4">
				<Pagination>
					<PaginationContent>
						<PaginationItem>
							<PaginationPrevious className="!text-normal" onClick={() => handlePageChange(Math.max(1, currentPage - 1))} disabled={currentPage === 1} />
						</PaginationItem>
							...
							if (page === 1 || page === totalPages || (page >= currentPage - 1 && page <= currentPage + 1)) {
								return (
									<PaginationItem key={page}>
										<PaginationLink className="!text-normal" isActive={page === currentPage} onClick={() => handlePageChange(page)}>
											{page}
										</PaginationLink>
									</PaginationItem>
								);
						...
						<PaginationItem>
							<PaginationNext className="!text-normal" onClick={() => handlePageChange(Math.min(totalPages, currentPage + 1))} disabled={currentPage === totalPages} />
						</PaginationItem>
					</PaginationContent>
				</Pagination>
			</div>
